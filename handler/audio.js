...
export const createTranscription = async (req, res) => {
    try {
        ...
        if (!recording) {
            return res.status(404).json({ error: `audio with id ${req.params.audioId} not found` });
        }
        ...
        // Load audio data
        let buffer = Buffer.from(await fetch(recording.audioUrl).then(x => x.arrayBuffer()))
        ...
        const result = await transcriber(audioData, { chunk_length_s: 30, stride_length_s: 5, return_timestamps: true });
