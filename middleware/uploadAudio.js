...
const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
        const allowedExtensions = /^(audio\/|.*\.)(m4a|flac|mp3|mp4|wav|wma|aac|opus|x-flac|x-wav|wave|mpeg)$/; // Regular expression for allowed extensions (case-insensitive)
        if (allowedExtensions.test(file.mimetype)) {
            return cb(null, true);
        }
        // console.log(file.mimetype);
        return cb(new Error('Invalid file type. Only audio files (m4a, flac, mp3, mp4, wav, wma, aac, opus) are allowed!'));
    },
